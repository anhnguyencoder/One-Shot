...
[ExecuteAlways]
public class LV9LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV9_ColosseumShowcase";
    ...
    [ContextMenu("Build LV9 Colosseum Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV9LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
    ...
    [ContextMenu("Clear LV9 Colosseum Landmark")]
    ...
    private void BuildColosseumPixelArt()
    {
        ...
        for (int x = -15; x <= 15; x++)
        {
            SpawnBlock(stonePrefab, x, baseY, z, $"Base_{x}");
            if (Mathf.Abs(x) <= 14)
            {
                SpawnBlock(stonePrefab, x, baseY + 1, z, $"BaseTop_{x}");
            }
        }
        ...
        for (int row = 0; row < 11; row++)
        {
            int gy = baseY + 2 + row;
            float t = row / 10f;
            int half = Mathf.RoundToInt(13f - (t * 4f));
            int innerHalf = Mathf.Max(1, half - 1);

            for (int x = -half; x <= half; x++)
            {
                bool outer = x == -half || x == half;
                bool ring = row == 0 || row == 3 || row == 6 || row == 9;
                bool cap = row == 10;

                bool archVoid = false;
                if (row <= 5)
                {
                    int[] archCenters = { -10, -6, -2, 2, 6, 10 };
                    for (int i = 0; i < archCenters.Length; i++)
                    {
                        if (Mathf.Abs(x - archCenters[i]) <= 1 && row <= 2)
                        {
                            archVoid = true;
                            break;
                        }
                ...
                bool build = outer || ring || cap;
                if (build && !archVoid)
                {
                    SpawnBlock(stonePrefab, x, gy, z, $"Wall_{x}_{gy}");
                }

                if (!outer && !cap && !archVoid && Mathf.Abs(x) == innerHalf && (row % 2 == 1))
                {
                    SpawnBlock(trimPrefab, x, gy, z, $"Pillar_{x}_{gy}");
                }
            }
        }
        ...
        {
            SpawnBlock(accentPrefab, x, baseY + 12, z, $"Torch_{x}");
        }
    }

    private void BuildArenaForegroundAndSky()
    {
