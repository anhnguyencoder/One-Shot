...
[ExecuteAlways]
public class LV13LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV13_StatueOfLibertyShowcase";
    ...
    [Header("2D Statue Of Liberty Mode")]
    ...
    [SerializeField] private int artPanelHeight = 25;
    ...
    [ContextMenu("Build LV13 Statue Of Liberty Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV13LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
        ...
        BuildApproachPath();
        Build2DArtPanel();
        BuildStatueOfLibertyPixelArt();
        BuildHarborForegroundAndSky();
    ...
    [ContextMenu("Clear LV13 Statue Of Liberty Landmark")]
    ...
    private void BuildStatueOfLibertyPixelArt()
    {
        GameObject stonePrefab = PickPrefab(cobblestonePrefab, minecraftCubePrefab, tntBlockPrefab);
        GameObject copperPrefab = PickPrefab(minecraftCubePrefab, cobblestonePrefab, tntBlockPrefab);
        ...
        for (int x = -18; x <= 18; x++)
        {
            SpawnBlock(stonePrefab, x, baseY, z, $"Island_{x}");
            if (Mathf.Abs(x) <= 15)
            {
                SpawnBlock(stonePrefab, x, baseY + 1, z, $"IslandTop_{x}");
            }
        }
        ...
        for (int y = baseY + 2; y <= baseY + 9; y++)
        {
            for (int x = -9; x <= 9; x++)
            {
                bool edge = x == -9 || x == 9 || y == baseY + 2 || y == baseY + 9;
                bool trim = y == baseY + 4 || y == baseY + 7;
                bool pillar = x == -6 || x == -3 || x == 0 || x == 3 || x == 6;
                bool doorwayVoid = Mathf.Abs(x) <= 1 && y <= baseY + 5;

                if ((edge || trim || pillar) && !doorwayVoid)
                {
                    SpawnBlock(stonePrefab, x, y, z, $"Pedestal_{x}_{y}");
                }
            }
        }

        int bodyBaseY = baseY + 10;
        for (int row = 0; row < 11; row++)
        {
            int gy = bodyBaseY + row;
            int center = -1;
            int half = row < 3 ? 4 : (row < 7 ? 3 : 2);

            for (int x = center - half; x <= center + half; x++)
            {
                bool shell = x == center - half || x == center + half;
                bool folds = ((x - center + row) & 1) == 0;
                if (shell || folds)
                {
                    SpawnBlock(copperPrefab, x, gy, z, $"Body_{x}_{gy}");
                }
            }
        }

        int headY = bodyBaseY + 10;
        for (int x = -2; x <= 0; x++)
        {
            SpawnBlock(copperPrefab, x, headY, z, $"HeadBase_{x}");
            SpawnBlock(copperPrefab, x, headY + 1, z, $"HeadTop_{x}");
        }

        SpawnBlock(accentPrefab, -1, headY + 2, z, "CrownCenter");
        SpawnBlock(accentPrefab, -3, headY + 2, z, "CrownLeft");
        SpawnBlock(accentPrefab, 1, headY + 2, z, "CrownRight");
        SpawnBlock(accentPrefab, -4, headY + 1, z, "CrownFarLeft");
        SpawnBlock(accentPrefab, 2, headY + 1, z, "CrownFarRight");

        for (int step = 0; step < 7; step++)
        {
            int gx = 2 + step;
            int gy = bodyBaseY + 6 + step;
            SpawnBlock(copperPrefab, gx, gy, z, $"Arm_{step}");
            if (step < 4)
            {
                SpawnBlock(copperPrefab, gx, gy - 1, z, $"ArmSupport_{step}");
            }
        }

        int torchX = 9;
        int torchY = bodyBaseY + 13;
        SpawnBlock(accentPrefab, torchX, torchY, z, "TorchCore");
        SpawnBlock(accentPrefab, torchX - 1, torchY, z, "TorchL");
        SpawnBlock(accentPrefab, torchX + 1, torchY, z, "TorchR");
        SpawnBlock(accentPrefab, torchX, torchY + 1, z, "TorchTop");

        for (int y = bodyBaseY + 5; y <= bodyBaseY + 8; y++)
        {
            SpawnBlock(copperPrefab, -6, y, z, $"TabletArm_{y}");
            SpawnBlock(copperPrefab, -5, y, z, $"TabletEdge_{y}");
        }
        ...
        for (int y = bodyBaseY + 5; y <= bodyBaseY + 8; y++)
        {
            for (int x = -9; x <= -7; x++)
            {
                bool edge = x == -9 || x == -7 || y == bodyBaseY + 5 || y == bodyBaseY + 8;
                if (edge || ((x + y) & 1) == 0)
                {
                    SpawnBlock(copperPrefab, x, y, z, $"Tablet_{x}_{y}");
                }
            }
        }
        ...
        for (int step = 0; step < 4; step++)
        {
            ...
            {
                SpawnBlock(stonePrefab, x, gy, z, $"FoundationStep_{step}_{x}");
            }
        }
    }

    private void BuildHarborForegroundAndSky()
    {
        ...
        for (int x = -20; x <= 20; x++)
        {
            if ((x & 1) == 0)
            {
                SpawnBlock(waterPrefab, x, artBaseY + 1, z, $"Harbor_{x}");
            }
        }
        ...
        int moonY = artBaseY + artPanelHeight - 5;
        SpawnBlock(accentPrefab, -13, moonY, z, "MoonCore");
        SpawnBlock(accentPrefab, -14, moonY, z, "MoonL");
        SpawnBlock(accentPrefab, -13, moonY + 1, z, "MoonTop");
        SpawnBlock(accentPrefab, -12, moonY + 1, z, "MoonTopR");
        SpawnBlock(accentPrefab, -12, moonY, z, "MoonR");
    }
