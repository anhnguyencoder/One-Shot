...
[ExecuteAlways]
public class LV8LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV8_SydneyOperaShowcase";
    ...
    [ContextMenu("Build LV8 Sydney Opera Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV8LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
    ...
    [ContextMenu("Clear LV8 Sydney Opera Landmark")]
    ...
    private void BuildSydneyOperaPixelArt()
    {
        GameObject basePrefab = PickPrefab(cobblestonePrefab, minecraftCubePrefab, tntBlockPrefab);
        GameObject sailPrefab = PickPrefab(minecraftCubePrefab, cobblestonePrefab, tntBlockPrefab);
        ...
        for (int x = -14; x <= 14; x++)
        {
            SpawnBlock(basePrefab, x, baseY, z, $"OperaPlinth_{x}");
            if (Mathf.Abs(x) <= 12)
            {
                SpawnBlock(basePrefab, x, baseY + 1, z, $"OperaPlinthTop_{x}");
            }
        }

        BuildSail(-7, baseY + 2, 7, z, sailPrefab, "SailL1");
        BuildSail(-3, baseY + 3, 8, z, sailPrefab, "SailL2");
        BuildSail(0, baseY + 4, 9, z, sailPrefab, "SailC");
        BuildSail(3, baseY + 3, 8, z, sailPrefab, "SailR2");
        BuildSail(7, baseY + 2, 7, z, sailPrefab, "SailR1");

        for (int x = -10; x <= 10; x += 2)
        {
            SpawnBlock(accentPrefab, x, baseY + 2, z, $"Window_{x}");
        }
    }

    private void BuildSail(int centerX, int baseY, int height, int z, GameObject prefab, string prefix)
    {
        for (int layer = 0; layer < height; layer++)
        {
            int gy = baseY + layer;
            int width = Mathf.Max(0, (height / 2) - Mathf.Abs(layer - (height / 2)));
            for (int x = centerX - width; x <= centerX + width; x++)
            {
                bool shell = x == centerX - width || x == centerX + width || layer == height - 1;
                if (!shell && (layer & 1) == 0)
                {
                    continue;
                }

                SpawnBlock(prefab, x, gy, z, $"{prefix}_{x}_{gy}");
            }
        }
    }

    private void BuildHarborWaterAndSky()
    {
        ...
        SpawnBlock(accentPrefab, 10, sunY, z, "SunCore");
        SpawnBlock(accentPrefab, 9, sunY, z, "SunL");
        SpawnBlock(accentPrefab, 11, sunY, z, "SunR");
        SpawnBlock(accentPrefab, 10, sunY + 1, z, "SunTop");
        SpawnBlock(accentPrefab, 10, sunY - 1, z, "SunBottom");
    }
