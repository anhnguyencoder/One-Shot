...
[ExecuteAlways]
public class LV16LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV16_MoaiShowcase";
    ...
    [ContextMenu("Build LV16 Moai Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV16LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
    ...
    [ContextMenu("Clear LV16 Moai Landmark")]
    ...
    private void BuildMoaiPixelArt()
    {
        ...
        for (int x = -20; x <= 20; x++)
        {
            SpawnBlock(stonePrefab, x, baseY, z, $"Coast_{x}");
            if (Mathf.Abs(x) <= 18)
            {
                SpawnBlock(grassPrefab, x, baseY + 1, z, $"GrassTop_{x}");
            }
        }

        BuildMoaiHead(-9, baseY + 2, z, stonePrefab, accentPrefab, "MoaiL");
        BuildMoaiHead(0, baseY + 3, z, stonePrefab, accentPrefab, "MoaiC");
        BuildMoaiHead(9, baseY + 2, z, stonePrefab, accentPrefab, "MoaiR");

        for (int x = -12; x <= 12; x++)
        {
            if ((x & 1) == 0)
            {
                SpawnBlock(stonePrefab, x, baseY + 2, z, $"Ahu_{x}");
            }
        }
        ...
        for (int step = 0; step < 4; step++)
        {
            ...
            {
                SpawnBlock(stonePrefab, x, gy, z, $"PlatformStep_{step}_{x}");
            }
        }
    }

    private void BuildMoaiHead(int centerX, int baseY, int z, GameObject stonePrefab, GameObject accentPrefab, string prefix)
    {
        for (int y = baseY; y <= baseY + 8; y++)
        {
            int half = y < baseY + 2 ? 2 : (y < baseY + 6 ? 3 : 2);
            for (int x = centerX - half; x <= centerX + half; x++)
            {
                bool edge = x == centerX - half || x == centerX + half;
                bool fill = ((x + y) & 1) == 0 || y >= baseY + 6;
                if (edge || fill)
                {
                    SpawnBlock(stonePrefab, x, y, z, $"{prefix}_Head_{x}_{y}");
                }
            }
        }
        ...
        {
            SpawnBlock(accentPrefab, centerX + 1, y, z, $"{prefix}_Nose_{y}");
        }

        SpawnBlock(accentPrefab, centerX - 1, baseY + 6, z, $"{prefix}_EyeL");
        SpawnBlock(accentPrefab, centerX + 1, baseY + 6, z, $"{prefix}_EyeR");
        SpawnBlock(accentPrefab, centerX, baseY + 9, z, $"{prefix}_TopKnot");
    }

    private void BuildPacificForegroundAndSky()
    {
        ...
        for (int x = -20; x <= 20; x++)
        {
            if ((x & 1) == 0)
            {
                SpawnBlock(waterPrefab, x, artBaseY + 1, z, $"Ocean_{x}");
            }
        }
        ...
        int cloudY = artBaseY + artPanelHeight - 8;
        for (int x = 6; x <= 10; x++)
        {
            SpawnBlock(cloudPrefab, x, cloudY, z, $"CloudA_{x}");
        }

        for (int x = -5; x <= -1; x++)
        {
            SpawnBlock(cloudPrefab, x, cloudY - 1, z, $"CloudB_{x}");
        }
    }
