...
[ExecuteAlways]
public class LV18LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV18_ParthenonShowcase";
    ...
    [ContextMenu("Build LV18 Parthenon Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV18LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
        ...
        BuildApproachPath();
        Build2DArtPanel();
        BuildParthenonPixelArt();
        BuildAegeanSkyAndGround();
    ...
    [ContextMenu("Clear LV18 Parthenon Landmark")]
    ...
    private void BuildParthenonPixelArt()
    {
        ...
        for (int x = -19; x <= 19; x++)
        {
            SpawnBlock(stonePrefab, x, baseY, z, $"AcropolisBase_{x}");
            if (Mathf.Abs(x) <= 16)
            {
                SpawnBlock(fillPrefab, x, baseY + 1, z, $"AcropolisTop_{x}");
            }
        }

        int hallMinX = -13;
        int hallMaxX = 13;
        int hallMinY = baseY + 2;
        int hallMaxY = baseY + 11;

        for (int y = hallMinY; y <= hallMaxY; y++)
        {
            for (int x = hallMinX; x <= hallMaxX; x++)
            {
                bool edge = x == hallMinX || x == hallMaxX || y == hallMinY || y == hallMaxY;
                bool beam = y == hallMinY + 3 || y == hallMinY + 7;
                bool column = x == -11 || x == -8 || x == -5 || x == -2 || x == 2 || x == 5 || x == 8 || x == 11;
                bool doorway = Mathf.Abs(x) <= 2 && y <= hallMinY + 4;

                if ((edge || beam || column) && !doorway)
                {
                    SpawnBlock(stonePrefab, x, y, z, $"Parthenon_{x}_{y}");
                }
            }
        }

        int pedimentBaseY = hallMaxY + 1;
        for (int layer = 0; layer < 4; layer++)
        {
            ...
            for (int x = -half; x <= half; x++)
            {
                ...
                bool fill = layer == 0 && (x % 2 == 0);
                if (edge || fill)
                {
                    SpawnBlock(stonePrefab, x, gy, z, $"Pediment_{x}_{gy}");
                }
            }
        }
        ...
        {
            SpawnBlock(accentPrefab, x, hallMaxY - 1, z, $"Frieze_{x}");
        }

        SpawnBlock(accentPrefab, -14, baseY + 3, z, "BrokenColumnL");
        SpawnBlock(accentPrefab, 14, baseY + 3, z, "BrokenColumnR");
    }
