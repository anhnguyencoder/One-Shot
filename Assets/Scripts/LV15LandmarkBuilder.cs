...
[ExecuteAlways]
public class LV15LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV15_MountFujiShowcase";
    ...
    [ContextMenu("Build LV15 Mount Fuji Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV15LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
    ...
    [ContextMenu("Clear LV15 Mount Fuji Landmark")]
    ...
    {
        GameObject rockPrefab = PickPrefab(cobblestonePrefab, minecraftCubePrefab, tntBlockPrefab);
        GameObject snowPrefab = PickPrefab(minecraftCubePrefab, cobblestonePrefab, tntBlockPrefab);
        ...
        for (int x = -20; x <= 20; x++)
        {
            SpawnBlock(rockPrefab, x, baseY, z, $"Foothill_{x}");
            if (Mathf.Abs(x) <= 18)
            {
                SpawnBlock(rockPrefab, x, baseY + 1, z, $"FoothillTop_{x}");
            }
        }
        ...
        for (int row = 0; row <= 14; row++)
        {
            ...
            for (int x = -half; x <= half; x++)
            {
                ...
                bool contour = ((x + row) & 1) == 0 || row >= 11;
                bool craterVoid = row >= 13 && Mathf.Abs(x) <= 1;
                if ((edge || contour) && !craterVoid)
                {
                    SpawnBlock(rockPrefab, x, gy, z, $"Mountain_{x}_{gy}");
                }
            }
        }
        ...
        {
            int gy = baseY + 2 + row;
            int half = Mathf.Max(1, (16 - row) - 2);
            for (int x = -half; x <= half; x++)
            {
                bool edge = x == -half || x == half;
                bool fill = ((x + gy) & 1) == 0 || row >= 12;
                if (edge || fill)
                {
                    SpawnBlock(snowPrefab, x, gy, z, $"SnowCap_{x}_{gy}");
                }
            }
        }
        ...
        {
            SpawnBlock(snowPrefab, 0, y, z, $"SnowRidge_{y}");
            if ((y & 1) == 0)
            {
                SpawnBlock(snowPrefab, -1, y - 1, z, $"SnowRidgeL_{y}");
                SpawnBlock(snowPrefab, 1, y - 1, z, $"SnowRidgeR_{y}");
            }
        }

        int pagodaX = -12;
        for (int y = baseY + 2; y <= baseY + 7; y++)
        {
            SpawnBlock(rockPrefab, pagodaX, y, z, $"PagodaCore_{y}");
        }

        for (int tier = 0; tier < 3; tier++)
        {
            int gy = baseY + 3 + (tier * 2);
            int half = 3 - tier;
            for (int x = pagodaX - half; x <= pagodaX + half; x++)
            {
                bool edge = x == pagodaX - half || x == pagodaX + half;
                bool roof = tier < 2 || x == pagodaX;
                if (edge || roof)
                {
                    SpawnBlock(accentPrefab, x, gy, z, $"PagodaRoof_{tier}_{x}");
                }
            }
        }

        SpawnBlock(accentPrefab, pagodaX, baseY + 8, z, "PagodaSpire");
    }

    private void BuildLakeAndSky()
    {
        ...
        for (int x = -20; x <= 20; x++)
        {
            if ((x & 1) == 0)
            {
                SpawnBlock(waterPrefab, x, artBaseY + 1, z, $"Lake_{x}");
            }
        }

        for (int x = -18; x <= 18; x += 6)
        {
            SpawnBlock(accentPrefab, x, artBaseY + 2, z, $"Ripple_{x}");
        }
