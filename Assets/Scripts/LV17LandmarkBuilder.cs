...
[ExecuteAlways]
public class LV17LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV17_ChichenItzaShowcase";
    ...
    [ContextMenu("Build LV17 Chichen Itza Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV17LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
        ...
        BuildApproachPath();
        Build2DArtPanel();
        BuildChichenItzaPixelArt();
        BuildJunglePlazaAndSky();
    ...
    [ContextMenu("Clear LV17 Chichen Itza Landmark")]
    ...
    private void BuildChichenItzaPixelArt()
    {
        GameObject stonePrefab = PickPrefab(cobblestonePrefab, minecraftCubePrefab, tntBlockPrefab);
        GameObject fillPrefab = PickPrefab(minecraftCubePrefab, cobblestonePrefab, tntBlockPrefab);
        ...
        for (int x = -20; x <= 20; x++)
        {
            SpawnBlock(stonePrefab, x, baseY, z, $"Plaza_{x}");
            if (Mathf.Abs(x) <= 17)
            {
                SpawnBlock(fillPrefab, x, baseY + 1, z, $"PlazaTop_{x}");
            }
        }

        for (int terrace = 0; terrace < 6; terrace++)
        {
            int y0 = baseY + 2 + (terrace * 2);
            int half = 14 - (terrace * 2);
            for (int y = y0; y <= y0 + 1; y++)
            {
                for (int x = -half; x <= half; x++)
                {
                    bool edge = x == -half || x == half || y == y0 + 1;
                    bool fill = ((x + y) & 1) == 0;
                    if (edge || fill)
                    {
                        SpawnBlock(stonePrefab, x, y, z, $"Pyramid_{terrace}_{x}_{y}");
                    }
                }
            }
        }

        int stairTopY = baseY + 13;
        for (int y = baseY + 2; y <= stairTopY; y++)
        {
            int width = Mathf.Max(1, 3 - ((y - (baseY + 2)) / 4));
            for (int x = -width; x <= width; x++)
            {
                SpawnBlock(accentPrefab, x, y, z, $"Stair_{x}_{y}");
            }
        }

        int templeBaseY = baseY + 14;
        for (int y = templeBaseY; y <= templeBaseY + 4; y++)
        {
            int half = y <= templeBaseY + 2 ? 4 : 3;
            for (int x = -half; x <= half; x++)
            {
                bool edge = x == -half || x == half || y == templeBaseY + 4;
                bool doorway = Mathf.Abs(x) <= 1 && y <= templeBaseY + 2;
                if ((edge || ((x + y) & 1) == 0) && !doorway)
                {
                    SpawnBlock(stonePrefab, x, y, z, $"Temple_{x}_{y}");
                }
            }
        }
        ...
        for (int x = -2; x <= 2; x++)
        {
            SpawnBlock(accentPrefab, x, templeBaseY + 5, z, $"TempleRoof_{x}");
        }
    }

    private void BuildJunglePlazaAndSky()
    {
        ...
        for (int x = -12; x <= -8; x++)
        {
            SpawnBlock(cloudPrefab, x, cloudY, z, $"CloudL_{x}");
        }

        for (int x = 5; x <= 9; x++)
        {
            SpawnBlock(cloudPrefab, x, cloudY - 1, z, $"CloudR_{x}");
        }
    }
