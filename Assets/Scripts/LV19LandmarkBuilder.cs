...
[ExecuteAlways]
public class LV19LandmarkBuilder : MonoBehaviour
{
    ...
    [SerializeField] private string mapRootName = "LV19_BurjKhalifaShowcase";
    ...
    [SerializeField] private int artPanelHeight = 26;
    ...
    [ContextMenu("Build LV19 Burj Khalifa Landmark")]
    public void BuildShowcase()
    {
        if (!TryResolvePrefabs())
        {
            Debug.LogError("LV19LandmarkBuilder: Could not resolve any usable block prefab.");
            return;
        }
        ...
        BuildApproachPath();
        Build2DArtPanel();
        BuildBurjKhalifaPixelArt();
        BuildDubaiSkylineAndDesert();
    ...
    [ContextMenu("Clear LV19 Burj Khalifa Landmark")]
    ...
    private void BuildBurjKhalifaPixelArt()
    {
        ...
        for (int x = -18; x <= 18; x++)
        {
            SpawnBlock(towerPrefab, x, baseY, z, $"Podium_{x}");
            if (Mathf.Abs(x) <= 15)
            {
                SpawnBlock(glassPrefab, x, baseY + 1, z, $"PodiumTop_{x}");
            }
        }

        for (int level = 0; level < 20; level++)
        {
            int gy = baseY + 2 + level;
            int half = Mathf.Max(1, 5 - (level / 4));

            for (int x = -half; x <= half; x++)
            {
                bool edge = x == -half || x == half;
                bool window = ((x + level) & 1) == 0;
                if (edge || window)
                {
                    SpawnBlock(towerPrefab, x, gy, z, $"Core_{x}_{gy}");
                }
            }

            if (level < 10)
            {
                int wingWidth = Mathf.Max(1, 4 - (level / 3));
                for (int x = -(wingWidth + 6); x <= -(wingWidth + 3); x++)
                {
                    if ((x + level) % 2 == 0)
                    {
                        SpawnBlock(glassPrefab, x, gy, z, $"WingL_{x}_{gy}");
                    }
                }

                for (int x = wingWidth + 3; x <= wingWidth + 6; x++)
                {
                    if ((x + level) % 2 == 0)
                    {
                        SpawnBlock(glassPrefab, x, gy, z, $"WingR_{x}_{gy}");
                    }
                }
            }
        }

        for (int y = baseY + 22; y <= baseY + 26; y++)
        {
            SpawnBlock(accentPrefab, 0, y, z, $"Spire_{y}");
        }

        for (int x = -4; x <= 4; x += 2)
        {
            SpawnBlock(accentPrefab, x, baseY + 21, z, $"Beacon_{x}");
        }
    }

    private void BuildDubaiSkylineAndDesert()
    {
        GameObject groundPrefab = PickPrefab(minecraftCubePrefab, cobblestonePrefab, tntBlockPrefab);
        ...
        for (int x = -20; x <= 20; x++)
        {
            if ((x & 1) == 0)
            {
                SpawnBlock(groundPrefab, x, artBaseY + 1, z, $"Dune_{x}");
            }
        }

        int[] skylineHeights = { 4, 6, 5, 7, 3, 6 };
        int sx = -16;
        for (int i = 0; i < skylineHeights.Length; i++)
        {
            for (int y = artBaseY + 2; y <= artBaseY + 1 + skylineHeights[i]; y++)
            {
                SpawnBlock(skylinePrefab, sx, y, z, $"Skyline_{i}_{y}");
            }
